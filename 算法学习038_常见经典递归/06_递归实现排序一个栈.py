# 为了方便,我们就使用python中collections包中的栈,来实现栈
from collections import deque


# 这个�?,也是�?能排�?,类型deque的栈(deque的实例化对象),当然这个�?�?以改�?
class Solution:
    def stack_sort(self, stack: deque) -> None:
        """
        🎯 栈排序算法（升序排序�?
        🔄 通过递归将栈元素按升序排列（栈底最小，栈顶最大）
        """
        depth: int = self.deep(stack)  # 📏 获取栈的当前深度
        while depth > 0:
            # 🔍 找到前depth�?元素�?的最大�?
            max_num = self.depth_max(stack, depth)
            # 🎰 计算最大值在前depth�?元素�?出现的�?�数
            k: int = self.times(stack, depth, max_num)
            # ⬇️ 将最大值沉底k�?
            self.down(stack, depth, max_num, k)
            # 📉 减少处理深度，跳过已处理的最大�?
            depth -= k

    # 📏 返回栈的深度
    # 💡 不改变栈的数�?状况 - 使用递归临时弹出再压�?
    def deep(self, stack: deque) -> int:
        """
        📊 计算栈的深度（递归实现�?
        """
        if not stack:
            return 0  # 🎯 基准情况：空栈深度为0

        num: int = stack.pop()  # 🎪 弹出栈顶元素
        depth: int = self.deep(stack) + 1  # 📈 递归计算剩余深度并加1
        stack.append(num)  # 🔄 恢�?�栈的原始状�?
        return depth

    # 🔍 返回栈前depth�?元素的最大�?
    def depth_max(self, stack: deque, depth: int) -> int:
        """
        🏆 在栈的前depth�?元素�?查找最大�?
        """
        if depth == 0:
            return -2147483648  # 🚫 基准情况：返回int最小�?

        num: int = stack.pop()  # 🎪 弹出当前元素
        rest_max: int = self.depth_max(stack, depth - 1)  # 🔄 递归查找剩余元素的最大�?
        max_num = max(num, rest_max)  # 🏅 比较当前元素和剩余元素的最大�?
        stack.append(num)  # 🔄 恢�?�栈的原始状�?
        return max_num

    # 🎰 计算最大值max_num在栈前depth�?元素�?出现的�?�数
    def times(self, stack: deque, depth: int, max_num: int) -> int:
        """
        🔢 统�?�最大值出现的次数
        """
        if depth == 0:
            return 0  # 🚫 基准情况：深度为0时�?�数�?0

        num: int = stack.pop()  # 🎪 弹出当前元素
        rest_times: int = self.times(stack, depth - 1, max_num)  # 🔄 递归统�?�剩余元素的次数
        times: int = rest_times + (1 if num == max_num else 0)  # �? 如果当前元素等于最大值则计数�?1
        stack.append(num)  # 🔄 恢�?�栈的原始状�?
        return times

    # ⬇️ 将最大值沉底操�?
    def down(self, stack: deque, depth: int, max_num: int, k: int) -> None:
        """
        �? 将最大值沉到栈底（排序关键步�?�）
        🎯 处理前depth�?元素，移除所有最大值，最后在底部添加k�?最大�?
        """
        if depth == 0:
            # 🎯 基准情况：�?�理完所有元素，在底部添加k�?最大�?
            for i in range(k):
                stack.append(max_num)  # �? 在栈底添加最大值（排序后的位置�?
        else:
            num: int = stack.pop()  # 🎪 弹出当前元素
            self.down(stack, depth - 1, max_num, k)  # 🔄 递归处理剩余元素

            if num != max_num:
                stack.append(num)  # 🔄 如果不是最大值，压回栈中
            # 💡 如果�?最大值，不压�? - 实现"沉底"效果


# 🧪 测试排序功能
def test_stack_sort():
    """
    �? 测试栈排序功�?
    """
    sol = Solution()

    # 🎯 测试用例1:乱序�?
    stack1 = deque([3, 1, 4, 2])
    print(f"📥 原栈: {list(stack1)}")
    sol.stack_sort(stack1)
    print(f"�? 排序�?: {list(stack1)}")
    print("🎯 期望: [1, 2, 3, 4] (栈底->栈顶)")
    print("---")

    # 🎯 测试用例2:已排序栈
    stack2 = deque([1, 2, 3, 4])
    print(f"📥 原栈: {list(stack2)}")
    sol.stack_sort(stack2)
    print(f"�? 排序�?: {list(stack2)}")
    print("---")

    # 🎯 测试用例3:逆序�?
    stack3 = deque([4, 3, 2, 1])
    print(f"📥 原栈: {list(stack3)}")
    sol.stack_sort(stack3)
    print(f"�? 排序�?: {list(stack3)}")
    print("---")

    # 🎯 测试用例4:有重复元素的�?
    stack4 = deque([2, 2, 1, 3, 2])
    print(f"📥 原栈: {list(stack4)}")
    sol.stack_sort(stack4)
    print(f"�? 排序�?: {list(stack4)}")
    print("🎯 期望: [1, 2, 2, 2, 3]")
    print("---")

    # 🎯 测试用例5:空栈
    stack5 = deque()
    print(f"📥 原栈: {list(stack5)}")
    sol.stack_sort(stack5)
    print(f"�? 排序�?: {list(stack5)}")
    print("---")


if __name__ == "__main__":
    # 🚀 运�?�测�?
    test_stack_sort()
